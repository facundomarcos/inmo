...
if(isset($_POST["id_prop"]) && !empty($_POST["id_prop"])){
 ...
 $input_codigo = trim($_POST["codigo"]);
    ...
    if(empty($codigo_err) && empty($tipo_err) && empty($operacion_err) && empty($ambientes_err) && empty($estado_err) && empty($direccion_err) ){
        ...
        $sql = "UPDATE propiedades SET tipo=?, codigo=?, operacion=?, ambientes=?, estado=?, direccion=?, adicional=?, imagen=?, activo=? WHERE id_prop=?";
    ...
    if(isset($_GET["id_prop"]) && !empty(trim($_GET["id_prop"]))){
        ...
        $id_prop =  trim($_GET["id_prop"]);
        
   
        $sql = "SELECT * FROM propiedades WHERE id_prop = ? and activo='S'";
        if($stmt = mysqli_prepare($link, $sql)){
          
            mysqli_stmt_bind_param($stmt, "i", $param_id_prop);
